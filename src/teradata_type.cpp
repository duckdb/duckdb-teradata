...
//
// 	struct VarcharField {
// 		static void Decode(BinaryReader &reader, Vector &vec, idx_t idx, bool is_null) {
// 			const auto length = reader.Read<uint16_t>();
// 			if(is_null) {
// 				FlatVector::SetNull(vec, idx, true);
// 			} else {
// 				//FlatVector::GetData<string_t>(vec)[idx] = StringVector::AddString(
// 			}
// 		}
// 	};
//
...
// 				FlatVector::SetNull(vec, idx, true);
// 			} else {
// 				FlatVector::GetData<T>(vec)[idx] = value;
// 			}
// 		}
//
// 		static void Encode(BinaryWriter &writer, const_data_ptr_t *src, bool is_null) {
// 			if(!is_null) {
// 				const auto value = FlatVector::GetData<T>(vec)[idx];
// 				writer.Write(value);
// 			}
// 		}
//
// 		static int32_t GetEncodedSize(const_data_ptr_t *src, bool is_null) {
// 			return sizeof(T);
// 		}
// 	};
// }
//
// class TeradataScanner {
// 	vector<TeradataType> tera_types;
// 	vector<LogicalType>  duck_types;
//
// 	DataChunk chunk;
//
// 	bool ScanRecord(char* record_ptr, idx_t record_len) {
//
// 		const auto null_bytes = (chunk.ColumnCount() + 7) / 8;
// 		const auto data_ptr = record_ptr + null_bytes;
// 		const auto data_len = record_len;
//
...
// 			const bool is_null = (record_ptr[byte_idx] & (1 << bit_idx)) != 0;
//
//
// 		}
// 	}
